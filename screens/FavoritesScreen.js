...
const FavoritesScreen = () => {
  ...
  const handleRefresh = useCallback(() => {
    // Trigger refresh action if you have one
    // dispatch(refreshFavorites());
  }, []);
  
  const navigateToDetails = useCallback((repo) => {
    navigation.navigate('DetailsScreen', { repo });
  }, [navigation]);
  
  // Memoize empty component to prevent unnecessary rerenders
        ...
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerBadge}>
            <Text style={styles.headerSubtitle}>
              {favoritesList.length} {favoritesList.length === 1 ? 'Repo' : 'Repos'}
            </Text>
          </View>
        </View>
      </Animated.View>
